...
class AnalyticsProcessor:
    ...
    def _get_date_range(self, time_range):
        """Returns start and end dates for the given time range"""
    ...
    def get_task_stats(self, username, time_range="all_time"):
        """Returns task statistics for the given time range"""
    ...
    def get_task_trends(self, username, time_range="all_time"):
        """Returns daily task completion data for line charts"""
        start_date, end_date = self._get_date_range(time_range)
    ...
    def get_task_forecast(self, username):
        ...
        # Get recent completion rate (last 7 days)
        today = datetime.date.today()
    ...
    def get_habit_stats(self, username, time_range="all_time"):
        """Returns habit statistics for the given time range"""
        start_date, end_date = self._get_date_range(time_range)
        ...
        for habit in habits:
            ...
            # Calculate completion rate for this habit in time range
            if start_date is None:
                # All time - get all tracking data
                with GetDBConnection(self.DBPath) as Conn:
                    Cursor = Conn.cursor()
                    Cursor.execute(
    ...
    def get_habit_trends(self, username, time_range="all_time"):
        """Returns daily habit completion data"""
        start_date, end_date = self._get_date_range(time_range)
        habits = self.HabitManager.GetUserHabits(username)
    ...
    def get_habit_streaks(self, username):
        """Calculates current and longest streaks per habit"""
        habits = self.HabitManager.GetUserHabits(username)
        streaks_data = {}
        ...
        for habit in habits:
            ...
            # Calculate streaks (consecutive days meeting target)
            current_streak = 0
            longest_streak = 0
            temp_streak = 0
    ...
    def get_habit_comparison(self, username):
        """Compares habit performance across time periods"""
        # Compare this week vs last week
        this_week_stats = self.get_habit_stats(username, "this_week")
        
        # Get last week stats manually
        today = datetime.date.today()
        days_since_monday = today.weekday()
        this_week_start = today - datetime.timedelta(days=days_since_monday)
        last_week_end = this_week_start - datetime.timedelta(days=1)
    ...
    def get_habit_forecast(self, username):
        """Predicts goal achievement dates based on current progress"""
        habits = self.HabitManager.GetUserHabits(username)
        forecasts = []
    ...
    def get_overall_trends(self, username, time_range="all_time"):
        ...
        return {
            ...
            "task_scores": [task_trends["completion_rates"][task_trends["dates"].index(d)] if d in task_trends["dates"] else 0 for d in all_dates],
