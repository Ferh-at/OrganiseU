...
from utils.ChartGenerator import (
    ...
    create_multi_line_chart, get_temp_chart_path
)
...
class AnalyticsWindow(customtkinter.CTkToplevel):
    ...
    def _create_dashboard_cards(self):
        """Create dashboard cards with key metrics"""
    ...
    def _CreateDashboardCard(self, parent, title, value, subtitle, color, row, col, command):
        """Create a dashboard card with drill-down capability"""
    ...
    def _create_productivity_trend_chart(self):
        """Productivity Trend - Multi-line Chart"""
        trends = self.processor.get_overall_trends(self.username, self.current_time_range)
    ...
    def _display_chart(self, chart_path, title):
        ...
        try:
            ...
            ChartImage = customtkinter.CTkImage(
                light_image=ChartImagePIL, size=(ChartImagePIL.width, ChartImagePIL.height)
            )
            ChartLabel = customtkinter.CTkLabel(
                ChartFrame, image=ChartImage, text=""
            )
    ...
    def _generate_insights(self):
        ...
        task_stats = self.processor.get_task_stats(self.username, self.current_time_range)
        if task_stats["total"] > 0:
            insights.append(f"✅ You've completed {task_stats['completed']} out of {task_stats['total']} tasks ({task_stats['completion_rate']}%)")
            if task_stats["pending"] > 0:
                insights.append(f"⏳ You have {task_stats['pending']} pending tasks to complete")
        ...
        if streaks:
            ...
            if longest_streak > 0:
                insights.append(f"🔥 Longest streak: {longest_habit} at {longest_streak} days")
        ...
        if forecasts:
            ...
            if on_track:
                ...
                insights.append(f"🎯 You're on track to complete '{habit_name}' by {projected_date}")
        ...
        if change > 0:
            ...
            insights.append(f"📉 Your completion rate decreased by {abs(change)}% this week")
        
        # Productivity score
        productivity = self.processor.get_productivity_score(self.username, self.current_time_range)
    ...
    def _get_task_details_content(self):
        ...
        for task_id, title, status, description in tasks:
            ...
            content.append(f"{status_emoji} {title} ({status})")
            if description:
    ...
    def _get_productivity_details_content(self):
        ...
        content.append(f"  Task Completion: {task_stats['completion_rate']}% (50% weight)")
        content.append(f"  Habit Completion: {habit_stats['avg_completion_rate']}% (50% weight)")
        content.append("")
        content.append("Calculation:")
        content.append(f"  ({task_stats['completion_rate']} × 0.5) + ({habit_stats['avg_completion_rate']} × 0.5) = {productivity}%")
