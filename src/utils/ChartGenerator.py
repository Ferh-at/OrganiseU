...
def _setup_style():
    """Configure matplotlib style to match app theme"""
...
def create_line_chart(data, title, xlabel, ylabel, save_path, dates=None):
    ...
    dates: list of date strings (optional, for x-axis labels)
    """
    _setup_style()
    fig, ax = plt.subplots(figsize=(10, 6), facecolor=COLORS["Dark"])
    
    if dates:
        # Convert date strings to formatted labels
...
def create_bar_chart(data_dict, title, xlabel, ylabel, save_path):
    """
    Create a bar chart
    data_dict: dictionary with keys as labels and values as bar heights
    """
    _setup_style()
    fig, ax = plt.subplots(figsize=(10, 6), facecolor=COLORS["Dark"])
...
def create_pie_chart(data_dict, title, save_path):
    """
    Create a pie chart
    data_dict: dictionary with keys as labels and values as sizes
    """
    _setup_style()
...
def create_multi_line_chart(data_dict, title, xlabel, ylabel, save_path, dates=None):
    ...
    data_dict: dictionary with keys as series names and values as lists of y-values
    dates: list of date strings (optional, for x-axis labels)
    """
    _setup_style()
    fig, ax = plt.subplots(figsize=(10, 6), facecolor=COLORS["Dark"])
...
def get_temp_chart_path(filename):
    """Generate a temporary file path for chart images"""
